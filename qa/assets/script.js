$(function () {
    ...
    $(".gq-run-btn").click(function () {
        ...
        if ($this.prev().is(".output")) {
            ...
            $output.removeClass().addClass("output bs-callout").html("<img src='/qa/assets/compiling.gif' />")
        } else {
            $code = $this.prev().find("textarea")
            $this.before("<div class='output bs-callout'><img src='/qa/assets/compiling.gif' /></div>")
            $output = $this.prev()
        }
        $.ajax({
            type: "POST",
